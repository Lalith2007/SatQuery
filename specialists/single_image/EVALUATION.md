...
**Evaluation Harness**: `specialists/single_image/training/colab/07_evaluate_qwen25vl.py` and `compare_paligemma_qwen.py`  
**Dataset**: `data/qwen_dataset/test.jsonl` (243 held-out samples)  
...
1. **Parent-Scene Spatial Isolation**: Every satellite crop belongs to an identified parent scene (e.g., `NH49E001017`). All crops from test parent scenes are completely excluded from the training and validation splits.
2. **Zero Split Leakage**:
   $$\text{Scenes}_{\text{Train}} \cap \text{Scenes}_{\text{Test}} = \emptyset, \quad \text{Overlap} = 0$$
